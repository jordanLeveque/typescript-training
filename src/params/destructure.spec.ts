describe('Destructuring', () => {
  ...
  describe('with Objects', () => {
    ...
    it('can destructure nested variables', () => {
      ...
      let addr = getAddress();
      let lat = addr.coords.lat,
      long = addr.coords.long;
  ...
  describe('with Arrays', () => {
    ...
    it('can skip indexes in arrays', () => {
      ...
      let nb = getNumbers();
      let one = nb[0], three = nb[2], six = nb[5];
      expect(one).toBe(1)
			expect(three).toBe(3)
			expect(six).toBe(6)
		})
  ...
  describe('combined together', () => {
      ...
      let addr = getData();
      let balance = process(addr.balance),
      funds = process(addr.funds),
      incomes = process(addr.incomes),
      payments = process(addr.payments);
